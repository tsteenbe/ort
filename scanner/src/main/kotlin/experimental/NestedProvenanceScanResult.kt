...
data class NestedProvenanceScanResult(
    ...
    fun filterByVcsPath(path: String): NestedProvenanceScanResult {
        ...
        require(getProvenances().all { it !is RepositoryProvenance || it.vcsInfo.path.isBlank() }) {
            "filterByVcsPath() requires that all provenances have no VCS path set."
        }

        val pathsWithinProvenances = getProvenances().filter {
            val provenancePath = getPath(it)
            provenancePath.isBlank() || provenancePath.startsWith("$path/")
        ...
        fun KnownProvenance.withVcsPath() =
            when (this) {
                is RepositoryProvenance -> {
                    ...
                    this.copy(vcsInfo = vcsInfo.copy(path = pathWithinProvenance))
                }
